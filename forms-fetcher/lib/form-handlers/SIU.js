...
exports.siuHandler = async function(form) {
	log.debug(`Вызван метод siuHandler`);
	...
	try {
		...
		log.error(`В методе siuHandler, возникла ошибка!`, err);
...
async function prepareBody(formData) {
	...
	const number = formData["contact_number"].replace(/[^\d]+/g, '').replace('996', '996-');
	body.fields['customfield_12501'] = number || '996-000000000'; // Контактный номер (Строка)
	...
	body.fields['customfield_15000'] = { id: formData["siu_category"] }; //Выбор категории (Строка)
