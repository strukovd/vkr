...
exports.vrHandler = async function(form) {
	log.debug(`Вызван метод vrHandler`);
	...
	try {
		...
		log.error(`В методе vrHandler, возникла ошибка!`, err);
...
async function prepareTitle(formData) {
	const res = [formData["account"], formData["email"], formData["contact_number"]].filter(Boolean).join(" ");
	return res;
}
