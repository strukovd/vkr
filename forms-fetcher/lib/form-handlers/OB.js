...
exports.obHandler = async function(form) {
	log.debug(`Вызван метод obHandler`);
	...
	try {
		...
		log.error(`В методе obHandler, возникла ошибка!`, err);
...
function generateSummary(row) { //TODO:
...
async function prepareBody(formData) {
	...
	const body = {
		"fields": {
			project: { id: "11005" },
			summary: 'Заявка с forms.gazprom.kg',
	...
	// body.fields['customfield_11916'] = formData["account"] || ''; //Лицевой счет
	...
	body.fields['customfield_14200'] = { id: "13200" };

	// let fullName = ``;
	// if(CLData.issues[0].fields.customfield_10107) fullName += CLData.issues[0].fields.customfield_10107;
	// if(CLData.issues[0].fields.customfield_10108) fullName += CLData.issues[0].fields.customfield_10108;
	// if(CLData.issues[0].fields.customfield_10109) fullName += CLData.issues[0].fields.customfield_10109;
	// body.fields['customfield_11806'] = fullName || ''; //ФИО
